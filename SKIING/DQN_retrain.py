...
MEMORY_SIZE = 8452*2
MAX_EPISODES = 4000
MAX_TIMESTEPS = 10 # No importa, era per mirar si enteniem els rewards
EPSILON = 0.99
EPSILON_DECAY = 0.999
GAMMA = 1 #0.99
BATCH_SIZE = 64
...
# DQN Agent class to control the training
class DQNAgent:
            ...
            while not gamedone:
                ...
                if gamedone:
                    ...
                    if mean_rewards > best_score_mean: # Save the best model
                        ...
                        torch.save(self.dnnetwork.state_dict(), "model_DQN_retrain.pth")
                    ...
                    torch.save(self.dnnetwork.state_dict(), "model_DQN_last_retrain.pth")
    ...
    def calculate_loss(self, batch):
        ...
        with torch.no_grad():
            next_q_values = self.target_network.get_qvals(next_states).max(1)[0]
...
pretrained_model_path = "model_DQN_last_manual.pth"
...
for i in range(15):
    get_gameplays_and_add_to_buffer(f"skiing_gameplay_data_{i}.pkl")
